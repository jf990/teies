/*+-------------------------------------------------------------------+
  ...
  |  <GLOBAL>  None                                                   |
  |                                                                   |
  |  <PARAM>   current   *odtrecord  INPUT   This is a pointer to an  |
  ...
  |                                                                   |
  |  <PURPOSE> To find the length of the segment that contains the    |
  |            given OCD. Must take into account Phantom segments and |
  |            hierarchys within segments.                            |
  |                                                                   |
...
sint odt_sgln ( current, obj_type )
...
if( ODT_POINTER_OK( current ) &&  ( obj_type > ANY_OBJ  )
                              &&  ( obj_type <= MAX_OBJ )  )
{
   ...
   if( odt_nxto( current ) != odt_prvo( current ) )
   {
             ...
             (odt_off(current) <= odt_off( odt + odt_nxto(current)))    )
      {
         ...
         if( odt_nxto( current ) <= ( current - odt ) )
         {
