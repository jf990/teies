/************************************************************************
    <DOC>
    <NAME>         a_crrd(OCB *atch_ocb, OID *act_oid)
    <LEVEL>        Activity
    <DATE>         Jan 26, 1988      March 1, 1989
    ...
    <PURPOSE>      This routine will traverse the screens necessary and
                   control the variables needed to create a READ activity
    <CALLS>        obj_opn(), obj_upd(), obj_find(), obj_get(), obj_all()
                   ...
                   t_spad(), t_delsp(), var_clr(), var_get(),
...

{
   ...
   while ( !flag )
   {
...

      {
...
#if 0
         /* #if will be taken out once priviledge options work   */
         ...
         /* #if will be taken out once sequencing works          */
         if ( s_comp( no, var_get(sequed), 1, 1, 'F', &xc ) != 0)
         {
            ...
            if (rc != 0)
            {
               except( ACCESS_DENIED_FOR_OBJECT, USER_ERROR, var_get(sequed),
                                         S_NULL, S_NULL, S_NULL, S_NULL);
         ...
         if ( s_comp( var_get(ab_loc), comp, 0, 0, 'F', &xc) != 0 )
         {
            ...
            {
#if 0
               /* once we decide how to handle items #if will be taken */
               /* out                                                  */
               rc = obj_item(var_get(ab_loc),what_to_get,&oid1,NIL,
                                         &bad_items, &num_bad, ANY_OBJ);
               if (rc != 0)
               {
                  except(ACCESS_DENIED_FOR_OBJECT, USER_ERROR,var_get(ab_loc),
                                     S_NULL, S_NULL, S_NULL, S_NULL);
...
#endif
               /* the following statements will be taken out once item */
               /* works                                                */
               if (edited1)
               {
...
#if 0
         /* once we figure out how to implement already created section*/
         /* tables the #if will be removed                             */
         if ( s_comp( var_get(sec_tab), comp, 0, 0, 'M', &xc) != 0 )
         {
            if ( s_comp( var_get(sec_tab), sp, 0, 0, 'F', &xc) != 0 )
            {
               ...
               if (rc != 0)
               {
                  except(ACCESS_DENIED_FOR_OBJECT, USER_ERROR,var_get(sec_tab),
                                     S_NULL, S_NULL, S_NULL, S_NULL);
               ...
               while ( !done )  /* still more sections need info */
               {
                      ...
                      ||  ( rc == USER_REQUESTED_PROCESS ) )
                  {
                     ...
                     if (s_comp(var_get(sec_loc),comp,0,0,'F',&xc) != 0)
                     {
                        ...
                        {
#if 0
                           /*once we can handle items this #if will be */
                           /* taken out                                */
                           rc = obj_item(var_get(sec_loc),what_to_get,
                              &oid3, NIL, &bad_items, &num_bad, ANY_OBJ);
                           if (rc != 0)
                           {
                              except(INPUT_EXCEPTION, USER_ERROR,
...
#endif
                           /* once items are handled  the following    */
                           /* statements will be taken out             */
                           if (edited3)
                           {
                     ...
                     if (t_valyn (var_get (ok_toadd)) == 1 )
                     {
                        if (edited3)
                        {
                           i++;   /* count for number of sections */
                           a_crpt(result_oid,act_ocb,i,&p_ocb,act_type);
                           obj_find( p_ocb, "COMMENT.TITLE", &tbl_line);
                           obj_cls(p_ocb);
                           s_sitos( &line, i );
                           s_pad(&line, line, 3, ' ', 'R');
         ...
         rc = a_slnot( act_ocb, rc_notf, READ);
