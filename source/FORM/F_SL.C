...
:PURPOSE.
         This routine will fill in a lis-information structure, which
         will then be placed on the top of the list stack as the current
         list type.  Until another list type is specified, all <li>
         items will be considered to apply to a simple list.  This
         routine will also incriment the number of simple lists that are
         presently active.
...

{
   ...
   if (CHK_MASK (f_fmat->status, HEADING))
    ...
    f_fmat->sl_level += 1;             /* new current ordered list lev */

    list.type  = SIMP_LIST;            /* code for ordered list        */
    list.level = f_fmat->sl_level;     /* gives current level of lists */
